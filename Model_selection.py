...
def score_predicteur(X,Y,classifieur,classifieur_fs,weight_fs,grid_fs,params_grid,metric_fs=log_loss,cv_main=KFold(n_splits=3,shuffle=True),cv_grid=KFold(n_splits=5,shuffle=True),smoothing_fs=lambda m:proc.smooth(m,window_len=5,window='hanning'),**kwargs):
    ...
    for IndexValidation,IndexTest in cv_main.split(X,Y):
        ...
        I = classifieur_fs.feature_select_from_model_weights(Xval,Yval,classifieur_fs,{"metric_fs":metric_fs},weight_fs,grid_fs,smoothing_fs)
